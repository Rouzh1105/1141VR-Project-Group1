...
public class Piece : MonoBehaviour {
    ...
    public Board board;
    public GameObject moveEffect, eatEffect;
    private float eatEffectTime = 0.5f;
    ...
    private float duration = 1f, height=1f;
    ...
    */ 
    ...
    public void movePiece(Square square) {
        Square pre_square = cur_square;
        Square eat_square = null;
        Piece eatenPiece = square.holding_piece;
        ...
        Vector3 targetPos = new Vector3(cur_square.coor.pos.x,transform.position.y,cur_square.coor.pos.z);
        if(pre_square != cur_square) StartCoroutine(JumpToPosition(targetPos, duration, height, (eat_square==null?null:eatenPiece)));
    }
    IEnumerator JumpToPosition(Vector3 targetPos, float duration, float jumpHeight, Piece eatenPiece)
    {
        GetComponent<Rigidbody>().isKinematic = true;
        Vector3 startPos = transform.position;
        float time = 0f;
        GameObject mf = this.gameObject;
        if (moveEffect != null) 
        {
            mf = Instantiate(moveEffect);
            mf.transform.position = targetPos + new Vector3(0.5f,0.5f,0.5f);
        }
        // 面向移動方向（可選）
    ...
    public bool checkValidCheckKingMove(Square square) {
        ...
        bool canEatCheckingPiece = false;

        // ★ 防呆：checking_pieces 可能沒有 key，或是 value 為 null
        if (board.checking_pieces != null &&
            board.checking_pieces.ContainsKey(team) &&
            board.checking_pieces[team] != null &&
            board.checking_pieces[team].cur_square != null)
        {
            canEatCheckingPiece = (square == board.checking_pieces[team].cur_square);
        }

        if (!board.isCheckKing(board.cur_turn) || canEatCheckingPiece)
        {
            avoids_check = true;
        }
    ...
    */
    ...
    private void eatPiece(Piece piece) {
        if (piece != null && piece.team != team) 
        {
            ...
            Debug.Log(this.transform.gameObject.name + " 吃 " + piece.transform.gameObject.name);
            piece.eatMe();
        }
    }
    public IEnumerator EatEffect()
    {
        if (eatEffect != null) 
        {
            GameObject e = Instantiate(eatEffect, this.transform);
            e.transform.position += new Vector3(0.5f, 1, 0.5f);
            yield return new WaitForSeconds(eatEffectTime);
            if (eatEffect != null)
            {
                Destroy(e);
            }
