...
public class Board : MonoBehaviour {
    ...
    void Start() {
        checking_pieces[-1] = null;
        checking_pieces[1] = null;
        aiController = GameObject.FindObjectOfType<ChessAI>();
        setBoardTheme();
    ...
    */
    ...
    public void hoverSelfSquare(Square square)
    {
        if (self_square) self_square.unHoverSquare();
        square.hoverSquare(themes[cur_theme].square_closest,true,true);
        self_square = square;
    }
    ...
    public void hoverValidSquares(Piece piece) {
        ...
        for (int i = 0; i < squares.Count ; i++) {
            if (piece.checkValidMove(squares[i])) {
                squares[i].hoverSquare(themes[cur_theme].square_hover,true);
    ...
    */ 
    ...
    public bool isCheckKing(int team)
    {
        ...
        for (int i = 0; i < pieces.Count; i++)
        {
            if (pieces[i].team != king.team)
            {
                ...
                {
                    if (checking_pieces.ContainsKey(team))
                        checking_pieces[team] = pieces[i];
                    else
                        checking_pieces.Add(team, pieces[i]);
    ...
    public bool IsKingAlive(int team)
    {
        for (int i = 0; i < pieces.Count; i++)
        {
            Piece p = pieces[i];
            if (p == null) continue;
            if (p.team == team && p.piece_name.Contains("King"))
            {
                return true;
            }
        }
        ...
        if (isCheckKing(team))
        {
            ...
            for (int i = 0; i < squares.Count; i++)
            {
                for (int j = 0; j < pieces.Count; j++)
                {
                    ...
                    if (pieces[j].team != team) continue;
    ...
    */ 
    ...
    public void doCheckMate(int loser) {
        ...
        string winner = (loser == 1) ? "You Win" : "You Lose";
